...
void Cleanup()
{
	byhj::Release<ID3DXMesh*>(Mesh);

	for(int i = 0; i < Textures.size(); i++)
		byhj::Release<IDirect3DTexture9*>( Textures[i] );

	byhj::Release<ID3DXMesh*>(SphereMesh);
	byhj::Release<ID3DXMesh*>(BoxMesh);
}
...
bool Display(float timeDelta)
{
	if( Device )
	{
		...
		D3DMATERIAL9 blue = byhj::BLUE_MTRL;
